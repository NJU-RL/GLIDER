...
class GLIDER_ONLINE:
    ...
    def train_in_domain(self,):
        ...
        torch.manual_seed(self.args['seed'] + self.engine.local_rank)
        ...
        for epoch in range(self.args['epochs']):
            ...
            for batch in dataloader:
                ...
                low_loss = self.update_low_policy(batch['low'])
                if online_update_cnt < self.args['update_k'] and self.buffer.ready():
                    online_q_loss, online_actor_loss = self.update_ac(self.buffer.sample(batch_size_per_gpu))
                    ...
                    if self.train_vari_list[task_id]==0:
                        task_id = (task_id+1)%len(self.task_names)
                        continue
                    vari_id = random.choice(range(0, self.train_vari_list[task_id]))   
    ...
    def train_out_domain(self, task_id, vari_id):
        batch_size_per_gpu = self.engine.train_micro_batch_size_per_gpu()
        torch.manual_seed(self.args['seed'] + self.engine.local_rank)
        sampler = DistributedSampler(self.buffer, 
                                     num_replicas=self.engine.world_size, 
